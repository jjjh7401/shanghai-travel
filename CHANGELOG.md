...
#### 사용자 매뉴얼 PDF 자동 생성 스크립트 (`scripts/generate-manual.mjs`)
- Playwright + 시스템 Chrome 기반 스크린샷 자동 캡처 (13장, 390×844 모바일 뷰포트)
- 한국어 8섹션 구성: 앱 개요·접속방법, 조 선택, 일정 보기, 일정 커스터마이징, 장소 상세, 지도 위치 확인, Amap 길찾기 연동, 여행 팁
- A4 PDF 생성 (`manual-output/상하이IBT여행가이드_사용매뉴얼.pdf`)
- 스크린샷 base64 임베드 방식으로 오프라인 단독 배포 가능

### 변경됨

#### 의존성
- `playwright` devDependency 추가 (PDF/스크린샷 생성용)
- `manual-output/` gitignore 등록 (생성 파일 제외)
